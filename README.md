...
Alpha 4 je distribuovaný síťový systém vyvinutý v Pythonu, který umožňuje překlad slov mezi spolupracujícími uzly v lokální síti. Pokud uzel (server) nedisponuje překladem pro zadané slovo, automaticky prohledá síť, identifikuje další běžící instance "Alpha 4" a požádá je o překlad.

## Klíčové funkce
* **Decentralizovaná architektura:** Každá instance funguje jako server i klient zároveň (P2P princip).
* **Automatické vyhledávání (Service Discovery):** Pomocí příkazů `TRANSLATEPING`/`TRANSLATEPONG` dokáže server v definovaném rozsahu IP adres a portů automaticky detekovat další aktivní uzly.
* **Distribuovaný překlad:** Pokud uzel slovo nezná, dotáže se ostatních validních peerů v síti.
* **Vlastní TCP protokol:** Komunikace probíhá skrze definovaný textový protokol (příkazy jako `TRANSLATELOCL`, `TRANSLATEDSUC`, `TRANSLATEDERR`).
* **Robustní konfigurace:** Flexibilní nastavení síťového rozsahu a portů přes `.ini` soubory.

## Technické řešení
* **Socket Programming:** Nízkoúrovňová správa TCP spojení (`socket` modul).
* **Síťová komunikace:** Implementace robustního timeoutu (5s) pro zajištění stability při skenování sítě.
* **Logging:** Detailní sledování aktivity a chybových stavů do `../logs/app.log`.
